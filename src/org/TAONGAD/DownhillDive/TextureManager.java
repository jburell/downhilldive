...
public class TextureManager{
	...
	private static TextureAtlas[] _loadQueue, _changeQueue, _removeQueue,
		_active;
	...
	public static void load(TextureAtlas textureAtlas){
		...
		Log.w("TEXTUREMANAGER",
			"TextureAtlas load queue is full max=" + Constants.MAX_TEXTURE_ATLAS_COUNT);
	}
	...
	public static void remove(TextureAtlas textureAtlas){
		...
		Log.w("TEXTUREMANAGER",
			"TextureAtlas remove queue is full max=" + Constants.MAX_TEXTURE_ATLAS_COUNT);
	}
	...
	public static void markForChange(TextureAtlas textureAtlas){
		...
		Log.w("TEXTUREMANAGER",
			"TextureAtlas change queue is full max=" + Constants.MAX_TEXTURE_ATLAS_COUNT);
	}
	...
	public static void setActive(TextureAtlas textureAtlas){
		...
		Log.w("TEXTUREMANAGER",
			"TextureAtlas limit reached max=" + Constants.MAX_TEXTURE_ATLAS_COUNT);
	}
	...
	public static void loadTextures(GL10 gl){
		for(i = 0; i < _loadQueue.length; i++){
			if(_loadQueue[i] != null){
				...
				Log.d("TEXTUREMANAGER", "Allocating new texture " + textures[0]);
				gl.glBindTexture(GL10.GL_TEXTURE_2D, textures[0]);
				setCurrentTexture(textures[0]);
				Bitmap bmp = Bitmap.createBitmap(_loadQueue[i].getWidth(),
					_loadQueue[i].getHeight(),
				...
				gl.glTexParameterf(GL10.GL_TEXTURE_2D,
					...
					_loadQueue[i].getOptions().minFilter);
				gl.glTexParameterf(GL10.GL_TEXTURE_2D,
					GL10.GL_TEXTURE_MAG_FILTER,
					_loadQueue[i].getOptions().magFilter);
				gl.glTexParameterf(GL10.GL_TEXTURE_2D,
				...
				gl.glTexEnvf(GL10.GL_TEXTURE_ENV,
					...
					_loadQueue[i].getOptions().texEnv);
				for(int j = 0; j < _loadQueue[i].getTextureArray().length; j++){
					if(_loadQueue[i].getTextureArray()[j] != null){
						if(_loadQueue[i].getTextureArray()[j].getType().getType() == TextureType.BYTEBUFFER){
							Log.d("TEXTUREMANAGER", "Copying in raw ByteArray texture");
							gl.glTexSubImage2D(GL10.GL_TEXTURE_2D,
								...
								_loadQueue[i].getTextureArray()[j].getHeight(),
								GL10.GL_RGBA,
				...
				Log.d("TEXTUREMANAGER", "Copying Complete");
		...
		Log.d("TEXTUREMANAGER", "Texture Loading Complete");
		_hasLoadQueue = false;
		System.gc();
		DownhillDive.getInstance().textureLoadComplete();
	}
