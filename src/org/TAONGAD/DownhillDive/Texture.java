...
public class Texture{
	...
	public Bitmap getBitmap(){
		...
		case TextureType.ASSET:
			try{
				bmp = BitmapFactory.decodeStream(DownhillDive.getInstance().getAssets().open(_textureType.getAssetPath()));
				return bmp;
			}catch(IOException e){
				Log.w("TEXTURE",
					"Texture asset not found, " + _textureType.getAssetPath());
				return null;
			}
		case TextureType.BITMAP:
			return _textureType.getBitmap();
		case TextureType.RESOURCE:
			bmp = BitmapFactory.decodeResource(DownhillDive.getInstance().getResources(),
				_textureType.getResourceId());
			return bmp;
		default:
			Log.w("TEXTURE", "Unkown TextureType, fatal error!");
			return null;
		}
	}
	...
	public Texture(String assetPath){
		...
		try{
			BitmapFactory.decodeStream(DownhillDive.getInstance().getAssets().open(_textureType.getAssetPath()),
				null,
				opts);
		}catch(IOException e){
			Log.w("TEXTURE", "Texture asset not found, " + assetPath);
		}
	...
	public Texture(int resourceId){
		...
		BitmapFactory.Options opts = new BitmapFactory.Options();
		opts.inJustDecodeBounds = true;
		BitmapFactory.decodeResource(DownhillDive.getInstance().getResources(),
			resourceId,
			opts);
		_width = opts.outWidth;
		_height = opts.outHeight;
