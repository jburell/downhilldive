...
public class TextureAtlas{
	...
	public TextureAtlas(int width, int height, TextureOptions textureOptions){
		...
		if(!isPowerOfTwo(_width) || !isPowerOfTwo(_height)){
			Log.w("TEXTUREATLAS",
				"TextureAtlas width and height must both be powers of two (2, 4, 8, 16, 32, 64, 128, 256, 512, 1024)");
		}
	...
	public void insert(Texture texture, int x, int y){
		...
		Log.w("TEXTUREATLAS",
			"TextureAtlas is full, attempted forced insertion max=" + Constants.MAX_TEXTURES_PER_ATLAS);
	}
	...
	public void insert(Texture texture){
		for(i = 0; i < _texture.length; i++){
			if(_texture[i] == null){
				...
				Log.w("TEXTUREATLAS",
					"TextureAtlas appears to have no room for your Texture?!");
				return;
			}
		}
		Log.w("TEXTUREATLAS",
			"TextureAtlas is full, attempted calculated insertion max=" + Constants.MAX_TEXTURES_PER_ATLAS);
	}
